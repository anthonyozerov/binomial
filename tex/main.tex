...
\chapter{The scientific process}

\begin{abstract}
We seek to understand, statistically, the process of estimation in science. How good is scientists' uncertainty quantification? How much between-study correlation is there? Previous metascience work has largely focused on the social and biomedical sciences and null-hypothesis significance testing (NHST). Much of physical science has an emphasis on measurement, rather than testing. In a problem like estimating the mass of a particle, there is no obvious null of ``no positive effect.'' 
\end{abstract}

\section{Introduction}

This study is largely exploratory in nature, with three main goals:
\begin{itemize}
  \item Characterize the observed distributions of study results.
  \item Gain a better understanding of how to interpret a single study result or a collection of results.
  \item Discuss possible solutions to the observed problems.
\end{itemize}
...
We think this collection of datasets is an important advance. As we will see, it enables a general investigation of the nature of data from ``independent'' experiments. But, as we will see in a later section, it also serves as a good real-world stress test of methods for meta-analysis in physics which doesn't rely on a simulation with friendly assumptions.
...
The speed of light has been the subject of hundreds of experiments since the 1600's. We collated a dataset of 157 such experiments, spanning from 1676 to 1978. We relied on three reports which collected many experiments to create our dataset \citep{birge1934velocity,froome1971velocity,raynaud2013determining}, and added two of the final determinations \citep{evenson1972speed,blaney1974measurement}.\footnote{An oft-repeated story is that the speed of light was consistently overestimated by supposedly-independent experiments for a time, until the bias was eventually corrected. This story (with the supposed correction happening around 1905) appears in \citet{rush1955speed}, which is, for example, cited in a response to \citet[pg.~1087]{stigler1977robust}. Our dataset, as visualized in Figure \ref{fig:historical}, does not reveal any highly-consistent pattern of over or under-estimation. A cursory look indicates that our dataset includes most of the results considered by \citet{rush1955speed}, though we also include over a hundred additional results.}
...
% We also separately include the 1970 PDG dataset described in Section \ref{sec:pdg1970} to investigate how similar it is to the full 2025 dataset. This is a useful check to see whether our results in Sections \ref{sec:historical-assumptions-check} and \ref{sec:realworld} might generalize to present-day PDG data.
...
We from from \citet{bailey2017not} a dataset of nuclear physics measurements from the BIPM Table of Radionuclides. A full description can be found in \citet[sec.~2.1-2.2, 3.4]{bailey2017not}, and complete references are in the supplementary material \citep{bailey2016data}.
...
\section{Distribution of scientific measurements}\label{sec:assumptions}
...
\end{equation}
If for an arbitrary $\mu$ ($\mu$ doesn't have to be the ``truth'' $\theta$) we have that $y_i\overset{\mathrm{ind}}{\sim}\mathcal{N}(\mu,\sigma_i^2)$ for all $i$, then $z_{ij}\sim|\mathcal{N}(0,1)|$ for all $i,j$. \citet{bailey2017not} and \citet{jeng2007bandwagon} perform analyses using these $z_{ij}$ values.

The next approach is normalized differences from a weighted mean. These are, as defined by \citet[eq.~3.3]{bailey2017not} and \citet[eq.~9]{roos1975new}:
\begin{equation}\label{eq:hi}
  h_i=\frac{|y_i-\bar{y}|}{\sqrt{\sigma_i^2+\sigma_{\bar{y}}^2}},
\end{equation}
where $\bar{y}$ and $\sigma_{\bar{y}}$ are defined in Equations \ref{eq:ybar} and \ref{eq:sigmaybar}.\footnote{Note that $\bar{y}$ is the point estimate we would obtain from Fixed Effect (Section \ref{sec:fixed-effect}) or the Birge Ratio (Section \ref{sec:birge}). The Birge Ratio is used by PDG.} \citet{roos1975new} directly use these $h_i$ residuals to approximate the distribution $|y_i-\theta|/\sigma_i$, but there are several issues with this approximation. One is that, because $y_i$ and $\bar{y}$ are correlated, it is not true that if $y_i\overset{\mathrm{ind}}{\sim}\mathcal{N}(\mu,\sigma_i^2)$ then $h_i\sim|\mathcal{N}(0,1)|$. In fact, if we let $S=\sum_{i=1}^n \frac{1}{\sigma_i^2}$ we will have:
\[y_i-\bar{y}\sim \mathcal{N}\left(0, \sigma_i^2\left(1-\frac{1}{\sigma_i^2S}\right)^2+\frac{S-\frac{1}{\sigma_i^2}}{S^2}\right).\]
Therefore, let's define a third measure $h_i^\prime$ which resolves this issue:
...
When collecting all $z_{ij}$ or $h_i$ in a particular dataset of measurements of quantities and building an empirical distribution, we must consider how to weigh them \citep[sec.~3.3]{bailey2017not}. We choose weights such that every \textit{quantity} in a dataset has equal weight---our interest in this study is on the level of meta-analyses, not on the level of individual measurements like \citet{bailey2017not}, who sets weights such that every \textit{measurement} has equal weight.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{../figs/measurement_dist.pdf}
  \caption{Distributions of the absolute value of the residuals of individual experiments between each other in pairs ($z_{ij}$: Equation \ref{eq:zij}), to a weighted mean ($h_i,h_i^\prime$: Equations \ref{eq:hi} and \ref{eq:hiprime}), and to a ground truth ($h_i^\star$: Equation \ref{eq:histar}). The black line corresponds to a folded Standard Normal. All of the datasets show much more mass in the tail than a Standard Normal distribution would have. The $z_{ij}$ lines in the panels with ``Bailey'' data are essentially a reproduction of \citet[fig.~2]{bailey2017not}, though weighting by quantity instead of by measurement.}
  \label{fig:measurement-dist}
\end{figure}

We show results for the distributions of $z_{ij}$, $h_i$ and $h_i^\prime$ in our datasets in Figure \ref{fig:measurement-dist}. As we can see, none of the datasets quite follow a Standard Normal distribution. The distributions are wider than an $\mathcal{N}(0,1)$ distribution, even though if it were true that $y_i\overset{ind}{\sim} \mathcal{N}(\mu,\sigma_i^2)$, the result for $z_{ij}$ and $h_i^\prime$ should be $|\mathcal{N}(0,1)|$. What can we conclude from this? Intuitively, there are unaccounted-for errors at play which make the $y_i$'s more widely dispersed than the given $\sigma_i$'s and a Normal assumption would suggest. Note that including the systematic errors given in the PDG datasets brings them much closer to a Normal distribution, but there is still some overdispersion. The radionuclide, psychology, and medical datasets have more overdispersion than the PDG when systematic errors are included.

What does this overdispersion mean in practice? The assumption $y_i\overset{ind}{\sim} \mathcal{N}(\mu,\sigma_i^2)$ is exactly the Fixed Effect assumption, where there is no unaccounted-for error. Therefore, in any of these datasets, performing inference under Fixed Effect assumptions would be suspect.

A critique of any analysis of the $z_{ij}$'s, $h_i$'s, or $h_i^\prime$'s is that it effectively measures the dispersion of the $y_i$ values around each other, not around the truth $\theta$. Other works do not explicitly use these, but measure dispersion of the $y_i$ values around a reference value which has a large uncertainty \citep{chen2003non} or is itself calculated from the $y_i$ values or a subset of them \citep{hanson2007lessons}. \citet{bukhvostov1973statistical,bukhvostov1997probability}, in his analysis of PDG data, attempts to avoid these issues by using old PDG values with newer ones as ground truths.\footnote{These articles face two issues:
\begin{itemize}
  \item They only heuristically take into account the uncertainty in the ``ground truth'' by excluding experimental results with relatively narrow uncertainties.
  \item There is no attempt to ensure that the experimental results are not included in the calculation for the ``ground truth.'' 
\end{itemize}} \citet[sec.~4]{shlyakhter1994improved} offers such an analysis in the context of forecasting, because for (relatively) short-term economic and demographic projections, the passage of time realizes a ground truth.
Our datasets with ground truths allow us to make our own attempt at this. In these datasets, we form the following measure:
\begin{equation}\label{eq:histar}
  h^\star_i = \frac{|y_i-\theta|}{\sqrt{\sigma_i^2+\sigma_\theta^2}},
\end{equation}
where $\sigma_\theta$ is our standard deviation for $\theta$, which is typically much smaller than $\sigma_i^2$ and may be zero. We show results for $h^\star_i$ for the historical, chemical, and PDG 1970 datasets in Figure \ref{fig:measurement-dist}. We see that the tails observed for $h^\star_i$ yield qualitatively similar results, which gives us some confidence in using the other measures to get an idea of the distribution around $\theta$. The only major difference is observed in the chemical dataset, where the relatively large $\sigma_\theta$ (see Figure \ref{fig:chemical}) yields smaller residuals. In the PDG 1970 dataset, we see an even heavier tail in $h^\star_i$ than in the other mesaures. As discussed by \citet[sec.~3.6]{bailey2017not}, positive correlations among measurements which will cause lighter tails in the measures which don't use an external reference value---we detect correlations in Section \ref{sec:independence}, so this may be the reason.

Stepping away from meta-analysis for a moment, we can consider the implications for science in general. Most of the time, we do not have multiple studies estimating the same quantity, so we can't empirically see any unaccounted-for errors. As suggested by \citet{borm2009evidence} in the context of medicine, we must be very careful when making conclusions based on only one study. The particular issue revealed in Figure \ref{fig:measurement-dist} is that, supposing that the measurements in our datasets are somewhat representative of results obtained in science, hypothesis test $p$-values or posterior probabilities coming from a Normal error model might be way off, especially when results lie in the tail of the distribution. In high-energy physics, a $5\sigma$ criterion (for two tails, $p\approx0.0000006$) is commonly used to make claims of discovery---this is much stricter than the $p=0.05$ convention commonly used in other fields.\footnote{\citet[sec.~3]{dorigo2015extraordinary} offers an account of how this $5\sigma$ standard arose.} Looking at Figure \ref{fig:measurement-dist}, we can repeat the observation of \citet{bailey2017not} that differences between results or between results and a ground truth at the ``$5\sigma$'' level can happen at rates from $0.001$ to upwards of $0.01$. This is because it is simply not true that, when a study gives $(y_i,\sigma_i)$, $y_i$ was really sampled from $\mathcal{N}(\theta,\sigma_i^2)$.

How can such heavy tails arise? This question is discussed in many works \citetext{e.g.~\citealp{bukhvostov1997probability}; \citealp{dorigo2015extraordinary}; \citealp[sec.~4.2-4.3]{bailey2017not}}. As observed by \citet{bailey2017not}, tails of the form observed in Figure \ref{fig:measurement-dist} cannot be produced by a simple multiplicative underestimation of uncertainties as suggested by BR: the model $y_i\sim\mathcal{N}(\theta,c\sigma_i^2)$. A Normal distribution would imply subgaussian tails, but Figure \ref{fig:measurement-dist} shows that tail probabilities decrease at an exponential or even slower-than-exponential rate.

Somewhat counterintuitively, the observed tails do not rule out the RE model $y_i\sim\mathcal{N}(\theta,\sigma_i^2+\tau^2)$ if the $\tau$ relative to $\sigma_i$ is allowed to vary across measurements (which must happen when the $\sigma_i$'s are unequal) or across quantities. Figure \ref{fig:measurement-dist-synth} shows results for a synthetic version of the PDG 2025 (stat+syst) dataset, where we have resampled points $y_i$ from a Normal distribution around zero using the given $\sigma_i$'s with $\tau$ set to half of the median $\sigma_i$ in the quantity. As we can see, this model is quite capable of generating tails like those we see in Figure \ref{fig:measurement-dist}. Even though the underlying distribution is Normal, the varying ratio of the true standard deviation to $\sigma_i$ frequently creates large residuals when the true standard deviation is relatively large.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{../figs/measurement_dist_synth.pdf}
  \caption{In PDG 2025 (stat+syst), the distributions of the absolute value of the residuals of individual experiments between each other in pairs ($z_{ij}$: Equation \ref{eq:zij}), to a weighted mean ($h_i,h_i^\prime$: Equations \ref{eq:hi} and \ref{eq:hiprime}), and to a ground truth ($h_i^\star$: Equation \ref{eq:histar}). The solid lines are the same as in the PDG 2025 (stat+syst) panel in Figure \ref{fig:measurement-dist}. The dashed lines are for synthetic data where $y_i$ is resampled from a Normal distribution around zero using the given $\sigma_i$'s and with $\tau$ set to half of the median $\sigma_i$ in the quantity. The black line corresponds to a folded Standard Normal. We see that a Random Effects model is able to produce fairly similar tails to those observed, even though the underlying distribution is Normal.}\label{fig:measurement-dist-synth}
\end{figure}
...
When measuring $\theta$ and obtaining an estimate $y$ (with, say, ``uncertainty'' $\sigma$), here is a brief list of processes we may go through:
\begin{itemize}
  \item Constructing an instrument, or designing an experiment, to measure $\theta$. The instrument or experiment may have known uncertainties (e.g.~a spring scale is calibrated to some degree of precision) but also unknown issues. For example, maybe we calibrated a spring scale at a certain temperature, but didn't know that the spring constant changes at different temperatures. Or, maybe we didn't realize that trains passing near our particle collider leak electric currents into the ground and affect our beam energy \citep{bravin1998influence}.
  \item A process of measurement which we might approximate as ``random.'' (e.g.~we weigh something a few times, and get a slightly different number each time).
  \item Some data cleaning or preprocessing (implemented in code which might have bugs).
  \item A statistical analysis of the data to obtain an estimate $(y,\sigma)$.\begin{itemize}
    \item The analysis certainly involves assumptions about the measurement process (e.g.~about the likelihood of a particular measurement for different values of $\theta$).
    \item Uncertainty quantification (obtaining $\sigma$) also involves assumptions about the measurement process, may involve some estimation, and may try to incorporate a ``non-random'' part of the uncertainty (e.g.~some accounting for the possible biases of an instrument). 
    \item The analysis may involve mathematical approximations (e.g.~a formula which is asymptotically correct).
    \item The analysis is likely executed using code (which might have bugs).
    \end{itemize}
\end{itemize}
Traditionally, uncertainty quantification simply takes a stochastic model of the measurement process and (perhaps through some approximations) obtains a frequentist confidence interval $y\pm\sigma$ for $\theta$, or (in a Bayesian analysis) uses a prior distribution to obtain a posterior distribution for $\theta$ or simply a credible interval $y\pm\sigma$. However, many other factors (approximations, coding errors, unknown systematic biases) may lead to our uncertainty quantification being somehow inadequate---often, we will underestimate the true uncertainty.

As we have seen, measurements in science follow a heavy-tailed distribution around each other and around the truth. Not only that, but their distributions around the truth and around each other have larger variances than we would expect if uncertainty was properly quantified. Many prior works have discussed a similar problem: study results are often not replicated in repeat experiments. This is usually discussed in the sense of hypothesis testing---an initial study rejects a null hypothesis, but a follow-up replication attempt does not. However, we think the perspective of measurement offers some additional insight. By stepping outside the framework of binary yes/no answers to scientific questions, type I and II error rates, etc., we are able to examine the distributions which study results follow.\footnote{A good example of looking at effect sizes (in medicine) is \citet{ioannidis2001evolution}, which, among other things, shows how early studies might show a very strong effect, but later studies often reduce the estimated effect size.} This framework is also perhaps more applicable to the physical sciences, where good measurement is a common aim. The resulting confidence intervals can be turned into rejections of hypotheses by the usual inversion, but these hypotheses are not what is directly of epistemic interest.

\citet{ioannidis2005most}, and many other works, discuss factors leading to the replication crisis:
\begin{itemize}
  \item Data dredging: performing many hypothesis tests and only reporting the significant ones without applying a multiple-testing correction.
  \item Garden of forking paths: With no preregistration of analysis methods, a researcher may be inclined by personal biases to perform an analysis that yields a hoped-for or expected result. This is more subtle than the above but is effectively a less-explicit multiple-testing.
  \item Publication bias: Null results are often not published, and ``exciting'' results (small p-value) may be more likely to be published. If many teams investigate a null hypothesis which happens to be false, with a $p$-value threshold of 0.05 it is likely that one of them will find a positive result---this will be the only one published.
  \item Low a-priori hypothesis probability: As an extreme case, if all hypotheses tested happen to be false, the literature will only contain true negatives and false positives. No true positives will appear.
  \item Real heterogeneity: details of a treatment may differ, and the populations may also differ in important ways, so the true average effect of a treatment may differ study-to-study.
\end{itemize}
In the context of measurement in the physical sciences, many of these don't make sense!
\begin{itemize}
  \item Data dredging: There are no hypotheses to test, we simply create confidence intervals for all of the effects.
  \item Garden of forking paths: If I am measuring the mass of a particle, I don't really care what it ends up being. Perhaps the only thing I care about is some agreement with prior results, so I don't look silly.
  \item Publication bias: If I spend $\$1\,000\,000$ to measure something, I will have to report the result no matter what I get!
  \item Low a-priori hypothesis probability: Again, we don't think in terms of hypotheses, but in terms of measurement. It doesn't make sense to talk about my measurement of something having a low a-priori probability.
  \item Real heterogeneity: physical quantities are (or are believed to be) well-defined constants, which anyone anywhere should in principle be able to measure with the right equipment.
\end{itemize}
Yet, we see basically the same things! So, we must consider why we see what we do in the physical sciences.
\begin{itemize}
  \item Instrument miscalibration.
  \item Approximation error in data analysis.
  \item The real statistical/sampling variability is not of a form which justifies CLT and a Normal approximation.
  \item The assumed statistical model differs from reality in an important way.
  \item The reported $\sigma$ itself is an estimated quantity, and often it may not be very well-constrained (especially when it somehow incorporates systematic errors).
  \item Along many steps in the pipeline, a coding error may introduce an unpredictable change in the final estimate.
\end{itemize}
...
One key issue that arises is unaccounted-for error, where an estimate of a quantity may have some error not taken into account in the given confidence interval. For example, perhaps the given interval only takes into account noise, and not the systematic errors which cannot be reduced by a taking larger sample. This error can be due to issues in the experiment like a non-randomly-sampled population, or an instrument which is miscalibrated, or analysis issues like approximations and model imperfections. But the unaccounted-for error doesn't have to be ``systematic:'' perhaps the noise is underestimated due to an incomplete understanding of the data-generating process. 

Empirically, unaccounted-for errors can be seen when results from different experiments/teams/methods are far outside of each other's error bars. Aggregating multiple estimates must therefore not only account for the modeled errors, but also somehow model the unaccounted-for errors in the different estimates. This leads to the counterintuitive observation that combining multiple estimates can sometimes \emph{increase} our uncertainty, rather than decreasing it---this is because a large unaccounted-for error is not apparent in only one estimate, and becomes apparent when we see more.
...
\item
  Do the standard deviations account for just statistical noise involved in their method, or some or all parts of their systematic errors?
\end{itemize}
...
Suppose we are given $n$ estimates $y_1,\ldots,y_n$ of a number $\theta$, from $n$ experiments or teams or methods.
% It is possible that each of these estimates has error from two sources: \textbf{systematic} and \textbf{statistical} (i.e.~noise). (For instance, we can have a data-generating process where $y_i$ is actually a noisy estimate of $\theta_i$, which is what experiment $i$ would see if it had no noise; because of systematic error, in general $\theta_i\neq \theta$).
Suppose we are further given a standard deviation $\sigma_i$ for each $y_i$. This could have two meanings:
\begin{itemize} 
  \item Experimenter $i$ believes that $[y_i-\sigma_i,y_i+\sigma_i]$ forms a $68.27\%$ confidence interval for $\theta$. That is, if the experiment were repeated many times, this interval would contain $\theta$ $68.27\%$ of the time.
  \item Having performed the experiment and obtained data $D_i$, experimenter $i$ has a posterior distribution $P(\theta|D_i)$ which is centered on $y_i$ and has standard deviation $\sigma_i$.
  % \item Experimenter $i$ believes that $\sigma_i$ is a good estimate for the standard deviation of a distribution $Y_i|\theta$ which $y_i$ was drawn from, and that $E[Y_i]=\theta$.
\end{itemize}

In this setting, how can we learn something about $\theta$? Without making any assumptions, we can't learn anything (for instance, we could have $y_i=\theta-a-\epsilon_i\ \forall i$. Even if we know that $\epsilon_i\sim\mathcal{N}(0,\sigma_i^2)$, we are unable to recover $\theta$ without knowing something about $a$). The assumptions will essentially be about whether the experimenters' beliefs (given above) about their $(y_i,\sigma_i)$ results are correct, the specific ways in which they could be incorrect, and how the results interact with each other.

The problem of learning something about $\theta$ is what we will consider in this paper. In some cases, we will suppose that we are not given $\sigma_i$'s. But for simplicity we will suppose being given no more than $(y_i,\sigma_i)$ pairs: no asymmetric uncertainties, no full predictive distributions, and no breaking down of $\sigma_i$ into multiple components.

\subsection{History}\label{sec:history}

The core statistical methods to aggregate data from different observations, and eventually different studies, developed first in the physical sciences. The method of ``least-squares'' (i.e., minimizing the sum of squared residuals to obtain a best-fit) was hinted at in the context of astronomy by Roger Cotes in his posthumously published \emph{Opera Miscellanea} \citeyearpar{cotes1722opera}.\footnote{He described that to calculate the position of an object from several observations with different errors, one must take the center of the positions as weighted by the inverse of the ``limits of the errors.'' This is the original text, which the present writer cannot read:
  \begin{quote}Ad eundem fere modum in aliis casibus Limites inveniuntur Errorum qui ex minus accuratis observationibus ortum ducunt, quin \& Positiones ad Observandum commodissimae deprehenduntur: ut mihi vix quidquam ulterius desiderari videatur postquam ostensum fuerit qua ratione Probabilitas maxima in his rebus haberi possit, ubi diversae Observationes, in eundem finem institutae, paullulum diversas ab invicem conclusiones exhibent. Id autem fiet ad modum sequentis Exempli. Sit p locus Objecti alicujus ex Observatione prima definitus, q, r, s ejusdem Objecti loca ex Observationibus subsequentibus; sint insuper P, Q, R, S pondera reciproce proportionalia spatiis Evagationum, per quae se diffundere possint Errores ex Observationibus singulis prodeuntes, quaeque dantur ex datis Errorum Limitibus; \& ad puncta p, q, r, s posita intelligantur pondera P, Q, R, S \& inveniatur eorum gravitatis centrum Z: dico punctum Z fore Locum Objecti maxime probabilem, qui pro vero ejus loco tutissime haberi potest. \hfill-- \citet[Aestimatio Errorum in Mixta Mathesi, pg. 22]{cotes1722opera}\end{quote}}
Astronomy was indeed a core motiving science for these developments through the 19th century. The simple observational problem of recording the position of an object (say, a planet or asteroid) across multiple days, then computing its orbit, requires combining multiple noisy estimates. Least squares was formalized by \citet[art. 179]{gauss1857theory} and stated in a more recognizable, modern form for simple one-dimensional experiment- or observation-aggregating by \citet[art. 69-71]{airy1861algebraical}. Effectively, this was the Fixed Effect model and method described below, which makes no attempt to account for any errors not modeled by the given $\sigma_i$'s.

As early as 1881, we see FW Clarke, a chemist, using least-squares to aggregate measurements of atomic weights reported by different experimenters who used different methods \citep{clarke1881abstract}. What is lacking from Clarke's work, though, is a statistical evaluation of systematic errors---he only took noise into account, and we see several weighted means with quoted uncertainties that are far too low when considering the spread of the different experimenters' results.\footnote{Consider the following results quoted for the atomic weight of chromium:
  \begin{quote}
...
% this section taken from baker2013meta p 113
...
The classical way to think of this model is that when you go out to measure $\theta$, you will actually be measuring a systematically biased $\theta_i$ (the ``random effect'') which is $\theta$ plus a realization of a random Normal; and this random Normal has the same standard deviation $\tau$ as those of all the other researchers, and does not have any relation with the level of noise $\sigma_i$. Your $y_i$ is then a noisy estimate of $\theta_i$, not of $\theta$.

Since in this paper we do not distinguish between systematic errors and statistical noise, we can instead think of this model as saying that the error not included in $\sigma_i^2$ is additive with it.

These assumptions could make sense if we think that all of the studies suffer from random unaccounted-for errors with the same standard deviation. The assumptions will not hold if the size of unaccounted-for errors is related to $\sigma_i$ (the size of the given error). They will also not hold if the unaccounted-for errors are not Normal (maybe there are outliers, or there is a sharper peak of experimenters with very little unaccounted-for error).
...
\begin{enumerate}
  ...
  \begin{algorithmic}
    ...
    \If{$Q(\hat\tau^2) < n-1 $} \Return $\hat\tau^2=0$
    \EndIf
    \While{$Q(\hat\tau^2)\not\approx n-1$}
      \State $w_i^\star\gets 1/(\sigma_i^2+\hat\tau^2)$ for all $i$; $\hat\theta\gets\frac{\sum_{i=1}^ny_iw_i^\star}{\sum_{i=1}^nw_i^\star}$
...
We don't know $c$ a priori. The idea is that the uncertainties $\sigma_i$ reported in the different experiments are too small (perhaps they only include noise, and not systematic error). So we can just expand the uncertainties by a multiplicative scaling factor.
...
\begin{enumerate}
\item $P(Y_i<\theta)=0.5$
\item The events $Y_i<\theta$ are independent.
\end{enumerate}
...
One caveat is that the Binomial distribution is discrete, so for finite $n$ the tail probabilities only take on a set of discrete values. This means that we cannot achieve exactly $0.6827$ ($1\sigma$) coverage without playing tricks like randomly shrinking or collapsing the interval, or (to get a little closer to $0.6827$) using asymmetric tails. But, for $n\geq 3$, we can always achieve conservative $\geq 0.6827$ coverage. \footnote{For $n=2$, we can only achieve 50\% by taking the range of the points.} See \ref{table-n} for examples of values of $n$ and the smallest coverage level $\geq0.6827$ which can be achieved exactly---usually, it is not too much higher.
...
\begin{enumerate}
\item
  $Y_i$ has a symmetric distribution with median $\theta$.
\item
...
\subsubsection{Birge ratio for forecasts}\label{sec:br-forecast}
...
\item
  ...
  \item
    If $\hat c \geq 1$, there may well be no irreducible uncertainty (or at least, it is small in comparison to the unaccounted-for error in estimating $\mu$), so we take $\hat\lambda^2=0$.
  \end{itemize}
\item
...
When $\theta$ is known, for a given $\beta$ we can find a likelihood-maximizing $\hat\tau_{\mathrm{MLE}}$ by searching over a grid of $\tau$ values (there is no closed-form solution). %TODO: confirm that there is no closed-form solution
...
Collecting the resulting likelihoods for different $\beta$'s, we find the likelihood-maximizing $\hat\beta_{\mathrm{MLE}}$ in any given dataset or set of datasets. Note that we restrict $\beta$ to $[0,1]$ as we seek to interpolate between RE and BR. We can also compute likelihood ratio confidence intervals for $\beta$. %TODO: clarify and check method for likelihood ratio confidence interval. Is df on chisq correct?
...
Let's perform an evaluation of different methods, using these standardized residuals to create synthetic data. In practice, for a fixed particle property, we don't expect the measurements to come from a distribution like that shown in \ref{fig:noground-std-resids}. Some particle properties have measurements showing large unaccounted-for errors and severe underestimation of $\sigma_i$, while others do not.

\section{Simulation study}\label{sec:simulation}

Let's do some simulations to benchmark the different meta-analysis methods we have discussed. In every simulation, we will have these parameters:
...
\item
  $\sigma_i^2$: the standard deviations of the noise (observed)
\item
  $\tau$: a parameter we control to vary the level of unaccounted-for error
  or underestimation of $\sigma_i$'s. A larger $\tau$ indicates larger
  unaccounted-for errors (i.e.~underestimation of $\sigma_i$'s).
\end{itemize}

A key issue in a simulation study is the distribution of the $\sigma_i$'s. Depending on how they are chosen, the $\sigma_i$'s could all be quite close, or quite different (e.g. with a large $\sigma_{\mathrm{max}}/\sigma_{\mathrm{min}}$). In their review of simulation studies of estimators for $\tau$ in random effects models, \citet[sec.~4]{langan2017comparative} find that the range of sample sizes (in effect, the range of $\sigma_i$'s) used in the simulation affects the performance of different meta-analysis methods. For example, \citet{inthout2014hartung} convincingly demonstrate that the HKSJ and DL estimators perform far worse when there is a large range of study sample sizes (effectively, large range of $\sigma_i$'s).

All of the methods in this study are location- and scale-invariant, so we don't lose anything of interest by fixing $\theta=0$ and the scale of the $\sigma_i^2$'s. The different experiments will vary $n$, the distribution of the unaccounted-for errors, and their scale relative to the $\sigma_i^2$'s.

Because $1\sigma$ intervals are commonly used in the sciences, we will set our target coverage for most methods to $0.6827$. But as we discussed in Sections \ref{sec:st} and \ref{sec:srt}, ST and SRT will not achieve exact coverage and will be conservative for an arbitrary choice of $(n,\alpha)$. We discussed some approaches to try to get closer to target coverage, but here we will stick with the vanilla versions. To provide a reasonable assessment of their exactness and interval lengths, for ST and SRT we will set their target coverages to levels close to or above $0.6827$ at which they will theoretically provide exact $1-\alpha$ coverage. For $n=10$ for ST, for example, this means targeting a coverage of $0.891$ (narrower symmetric intervals could achieve $0.656$---just below our stated minimum of $0.6827$).
...
\begin{itemize}
\item
  $\theta_i\overset{\mathrm{iid}}{\sim}\mathcal{N}(0,\tau^2)$: the unaccounted-for errors
\item
...
\begin{itemize}
\item
  $\theta_i\overset{\mathrm{iid}}{\sim}\tau\cdot \mathrm{Cauchy}(0,1)$: the unaccounted-for errors
\item
  $y_i\overset{\mathrm{ind}}{\sim}\mathcal{N}(\theta_i,\sigma_i^2)$: the estimates (observed)
\end{itemize}
...
This is like Random Effects, but where the unaccounted-for errors are correlated between experiments. It will violate the assumptions of all the methods we have presented. Note that due to the computational difficulty of sampling a high-dimensional multivariate Normal, we will not experiment with $n>100$ under this model.
...
\item
  Under the Random Effects model, RE yields better coverage with smaller intervals when the unaccounted-for errors are small, but does not achieve coverage when unaccounted-for errors are comparable to or larger than noise. This is surprising until we remember that inference for the Random Effects model is approximate, not exact. BR performs very poorly, espeially for large $n$.
\item
...
The main message from the simulations is that, by using nonparametric methods which ignore the $\sigma_i$'s, much robustness is gained, and (usually) little precision lost. The primary exception to this is when unaccounted-for errors are quite small and the scales of the noise vary widely. Intuitively, this is the case when the $\sigma_i$'s are most informative, and the most can be gained from using them.

But when unaccounted-for errors are large, or unaccounted-for errors are within the same order of magnitude of each other, it seems not much is gained by incorporating the $\sigma_i$ information.
...
We have just performed a simulation study to examine how different methods perform under different assumed data-generating processes. However, when doing a real meta-analysis, none of the above assumed data-generating processes will be true. To get a more realistic handle on the performance of the different methods above, we can examine whether or not the different methods can recover, using old studies, values $\theta$ which now have precise knowledge of. We will use the datasets with ground truths described in Section \ref{sec:data} towards this purpose.
...
\item Even the weaker assumptions made by these methods do not hold in real-world data. In particular, in Section \ref{sec:independence}, we were able to detect violations of the independence assumption in a real-world dataset. However, as we demonstrate in Sections \ref{sec:simulation} and \ref{sec:realworld}, this can be accomodated with a small modification to the Sign Test.
\end{itemize}

\subsection{Should we use statistics for this in the first place?}

Let's return to the case of IMBIE. We found that the different methods gave a range of about $-75$ to $-200$ $\mathrm{Gt/yr}$. Consider the hypothesis ``Antarctic mass balance is between $-75$ to $-200$ $\mathrm{Gt/yr}$.'' ST gives us a 75\% confidence interval which contains this hypothesis. What should our personal posterior belief be, or the IPCC's?
