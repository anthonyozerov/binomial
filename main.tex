...
Stepping away from meta-analysis for a moment, we can consider the implications for science in general. Most of the time, we do not have multiple studies estimating the same quantity, so we can't empirically see any unaccounted-for errors. Supposing that the measurements in our datasets are somewhat representative of results obtained in science, we must be careful when offering support for claims via hypothesis tests or posterior probabilities when using a Normal error model, especially when we make claims about very small probabilities in the tail of the distribution. In high-energy physics, a $5\sigma$ criterion $(p=0.00000029)$ is commonly used to make claims of discovery---this is much stricter than the $p=0.05$ convention commonly used in other fields.\footnote{\citet[sec.~3]{dorigo2015extraordinary} offers an account of how this $5\sigma$ standard arose.} Looking at Figure \ref{fig:measurement-dist}, we can repeat the observation of \citet{bailey2017not} that differences between results or between results and a ground truth at the ``$5\sigma$'' level can happen at rates from $0.001$ to upwards of $0.01$. This is because it is simply not true that, when a study gives $(y_i,\sigma_i)$, $y_i$ was really sampled from $\mathcal{N}(\theta,\sigma_i^2)$.

How can such heavy tails arise? \citet{dorigo2015extraordinary} and \citet[sec.~4.2-4.3]{bailey2017not} both discuss this question. As observed by \citet{bailey2017not}, tails of the form observed in Figure \ref{fig:measurement-dist} cannot be produced by a simple multiplicative underestimation of uncertainties as suggested by BR: the model $y_i\sim\mathcal{N}(\theta,c\sigma_i^2)$. A Normal distribution would imply subgaussian tails, but Figure \ref{fig:measurement-dist} shows that tail probabilities decrease at an exponential or even slower-than-exponential rate.
